...
fun <DFAState> getDFAForEmptyLanguage(): DFA<DFAState> = TODO("DFA interface does not allow for empty language")
...
fun <DFAState> DFA<DFAState>.getReachableStates(): Set<DFAState> {
    ...
    return getReachable(listOf(getStartingState()), graph)
}
...
fun <DFAState> DFA<DFAState>.getAliveStates(): Set<DFAState> {
    ...
    return getReachable(getAllStates().filter(this::isAccepting), graph)
}
